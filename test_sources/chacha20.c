// Modified from https://github.com/Ginurx/chacha20-c
...
static void chacha20_init_context(struct chacha20_context *ctx, uint8_t key[], uint8_t nounc[], uint64_t counter);

static void chacha20_xor(struct chacha20_context *ctx, uint8_t *bytes, size_t n_bytes);

static __always_inline uint32_t rotl32(uint32_t x, int n)
{
  return (x << n) | (x >> (32 - n));
}

static __always_inline uint32_t pack4(const uint8_t *a)
{
  uint32_t res = 0;
...
static __always_inline void unpack4(uint32_t src, uint8_t *dst)
{
  dst[0] = (src >> 0 * 8) & 0xff;
  dst[1] = (src >> 1 * 8) & 0xff;
  dst[2] = (src >> 2 * 8) & 0xff;
...
static __always_inline void chacha20_init_block(struct chacha20_context *ctx, uint8_t key[], uint8_t nonce[])
{
...
static __always_inline void chacha20_block_set_counter(struct chacha20_context *ctx, uint64_t counter)
{
  ctx->state[12] = (uint32_t)counter;
  ctx->state[13] = pack4(ctx->nonce + 0 * 4) + (uint32_t)(counter >> 32);
}

static __always_inline void chacha20_block_next(struct chacha20_context *ctx)
{
...
static __always_inline void chacha20_init_context(struct chacha20_context *ctx, uint8_t key[], uint8_t nonce[], uint64_t counter)
{
...
static __always_inline void chacha20_xor(struct chacha20_context *ctx, uint8_t *bytes, size_t n_bytes)
{
...
void do_encrypt()
{
  unsigned long stackAddr = 0x10000;
  asm volatile("r10 = %0"
               :
               : "r"(stackAddr));
  uint8_t *buffer = *(uint8_t **)0x10;
  size_t bufferSize = *(size_t *)0x18;
  uint8_t *key = *(uint8_t **)0x20;
  uint8_t *nonce = *(uint8_t **)0x28;
  uint64_t counter = *(uint64_t *)0x30;
